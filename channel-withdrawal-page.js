  ...
  window.HALO_CHANNEL_WITHDRAWAL = {
    create({ state, persist, synchronize, current, actions, feedback, escape: e }) {
      ...
      function render() {
        ...
        const notes = done ? '<p class="withdrawal-footnote">需要再次申请时，可从申请进度重新开始。</p>' : '<section class="withdrawal-retained"><span aria-hidden="true">✓</span><div><h3>这些会保留</h3><p>已提交资料、审核及学习记录。<br>会员身份、积分和健康功能不受影响。</p></div></section><p class="withdrawal-footnote">撤回后不能继续本次申请。重新申请需再次填写资料并签约，商学院学习进度继续保留。</p>';
