(function () {
  ...
  const entry = (title, body, meta, action, tone = "") => `<button class="entry-card ${e(tone)}" data-action="${e(action)}"><span class="entry-symbol" aria-hidden="true"></span><span class="entry-copy"><small>${e(meta)}</small><strong>${e(title)}</strong><span>${e(body)}</span></span><i aria-hidden="true">›</i></button>`;
  ...
  const task = (title, reward, status, action, tone = "") => `<button class="task-row" data-action="${e(action)}"><span class="task-copy"><strong>${e(title)}</strong><span>${e(reward)}</span></span>${pill(status, tone)}</button>`;
  ...
  function member(item, ctx) {
    ...
    const pages = {
      "MEM-01": () => shell(item, "会员", `<section class="member-hero"><div class="identity-line"><span>${e(levelCode)}</span><small>当前等级</small></div><h2>${e(currentLevel)}</h2><p>${active ? "今天再佩戴 1 小时 18 分，即可完成有效佩戴任务。" : retained ? "已有等级、成长和徽章已保留；重新激活后继续记录未来成长。" : "会员权益已开启；激活 Halo Ring 后开始记录未来成长。"}</p><div class="hero-facts"><div><span>HALO成长值</span><strong>${e(growth.toLocaleString())}</strong></div><div><span>Halo Points</span><strong>18,800</strong></div></div></section>${active ? `<section class="next-action-card"><span>今天最接近完成</span><h3>再佩戴 1 小时 18 分</h3><p>完成后可获得 8 HALO成长值与 20 Halo Points。</p>${progress(89, "今日有效佩戴")}${actions([["查看任务进度", "go:MEM-05", "primary"]])}</section>` : feedback(retained ? "成长已暂停" : "成长尚未开始", retained ? "重新绑定并激活 Halo Ring 后，继续记录新的成长。" : "绑定并激活 Halo Ring 后，从那一刻开始记录新的成长。", "warm")}<section class="entry-section"><h3>会员账户</h3>${entry("等级与权益", `${currentLevel} · 查看下一等级`, "当前身份", "go:MEM-02", "warm")}${entry("Halo Points", "余额、临期提醒与兑换", "18,800 可用", "go:PTS-01")}${entry("成长徽章", "查看长期习惯留下的记录", badgeMeta, "go:MEM-06")}</section><section class="entry-section"><h3>更多</h3>${entry("全部任务", "今天、本周与本月", active ? "持续成长" : "需激活 Halo Ring", "go:MEM-04")}${entry("邀请朋友", "分享 Halo，查看推荐进度", "会员推荐", "go:REF-01")}</section>`),
      ...
      "PTS-01": () => shell(item, "Halo Points", `<section class="points-hero ${state.pointsMode === "pending" ? "blocked" : ""}"><small>${state.pointsMode === "pending" ? "当前可用" : "可用 Halo Points"}</small><h2>${state.pointsMode === "pending" ? "0" : "18,800"}</h2><p>${state.pointsMode === "pending" ? "仍有 1,200 Points 待处理" : "符合条件的订单最高可抵扣约 ¥188 · 不可提现"}</p></section>${state.pointsMode === "pending" ? `${feedback("Halo Points 使用暂时暂停", "之后新获得的 Points 会先完成这次调整；完成前不能抵扣或兑换，也不会向你追讨现金。其他功能不受影响。", "danger")}${actions([["查看调整详情", "go:PTS-02", "primary"], [state.pointsAppealSubmitted ? "申诉已提交" : "联系客服申诉", state.pointsAppealSubmitted ? "" : "commercial:points-appeal", "secondary", state.pointsAppealSubmitted]])}` : `<section class="points-glance"><div><span>90 天内到期</span><strong>2,400</strong></div><div><span>30 天内到期</span><strong>800</strong></div></section><section class="entry-section">${entry("Halo Points 兑换", "内容、体验与活动名额", "去使用", "go:PTS-03", "warm")}${entry("Halo Points 明细", "奖励、使用与到期记录", "查看全部", "go:PTS-02")}</section>${disclosure("Halo Points 怎么使用", `<p>普通商品每单最多抵扣现金售价的 30%；指定内容可以使用全额 Halo Points 兑换。Halo Points 不能充值、提现或折现。</p>`)}`}`),
      ...
      "REF-01": () => shell(item, "会员推荐", `<section class="referral-card"><small>会员推荐</small><h2>邀请朋友了解 Halo</h2><p>好友通过你的推荐完成合资格订单、注册和 Halo Ring 激活，并通过订单有效性确认后，你会收到 20,000 Halo Points。${active ? "你的 Halo Ring 当前已激活，还会获得 150 HALO成长值。" : "你的 Halo Ring 当前未激活，因此不获得 150 HALO成长值；之后重新激活也不会补发。"}</p></section>${state.referralQrShown ? `<section class="qr-card referral-code-card"><span class="referral-code" aria-label="推荐码 HALO-8K2M">HALO<br>8K2M</span><strong>你的推荐码</strong><span>好友可在支付前确认来自你的推荐</span></section>` : ""}<section class="referral-status"><div><strong>2</strong><span>今年有效推荐</span></div><div><strong>1</strong><span>正在进行</span></div></section>${entry("好友已支付，等待激活", "完成激活并通过订单确认后发放奖励", "进行中", "go:SEL-10", "warm")}${disclosure("哪些推荐可以获得奖励", `<p>合资格推荐需通过实名和订单有效性确认。取消、全额退款、待处理售后、自购、关联账户互推或异常订单不奖励；每个自然年最多奖励 12 位有效新会员。</p>`)}${disclosure("订单来源怎样确认", `<p>好友可在支付前主动修改一次订单来源并再次确认；支付后不能更改。同一订单只记录一种来源，会员推荐奖励与渠道现金收益不会同时产生。</p>`)}${actions([[state.referralQrShown ? "隐藏推荐码" : "显示推荐码", "commercial:referral-qr", "primary"], ["返回会员中心", "go:MEM-01", "secondary"]])}`),
    };
  ...
  function select(item) {
    ...
    const pages = {
      ...
      "SEL-13": () => shell(item, "售后进度", `${feedback(state.afterSaleStatus === "submitted" ? "申请已提交" : "退款处理中", state.afterSaleStatus === "submitted" ? "预计反馈时间会在状态更新后显示。" : "现金会按原支付路径退回；Halo Points 会按页面显示的使用期恢复。", state.afterSaleStatus === "submitted" ? "sage" : "warm")}${stepper(["提交申请", "审核与寄回", "退款完成"], 1)}${summary([["售后单号", "AS20260901004"], ["现金退款", "¥349 · 原路退回"], ["Halo Points 恢复", "3,000"], ["当前状态", "处理中"]])}${state.afterSaleUploadSelected ? feedback("凭证已添加", "receipt-photo.jpg · 1.8 MB", "sage") : ""}${actions([[state.afterSaleUploadSelected ? "更换凭证" : "补充凭证", "commercial:aftersale-upload", "secondary"], ["联系 Halo 售后", "go:HELP-03", "primary"]])}`),
    };
  ...
  function channel(item) {
    ...
    const specific = {
      "CHN-01": `<section class="channel-intro"><small>Halo 体验顾问</small><h2>申请成为 Halo 体验顾问</h2><p>完成身份确认、Halo Ring 激活、基础学习、审核以及协议和收款资料后，收到身份生效通知，才能使用顾问工具。购买或激活设备不会自动获得渠道身份。</p></section>${actions([["开始申请", "go:CHN-02", "primary"]])}<button class="inline-page-link" data-action="go:CHN-11">查看已有申请进度</button>${stepper(["确认身份", "完成基础学习", "提交审核", "协议与收款"], 0)}${feedback("顾问等级从 L1 开始", "顾问等级与会员等级相互独立，不会自动转换。", "plain")}`,
